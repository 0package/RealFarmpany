...
#device
GPIO.setup(led_pin, GPIO.OUT, initial=GPIO.LOW)     #LED
GPIO.setup(fan_pin, GPIO.OUT, initial=GPIO.LOW)     #Fan
GPIO.setup(cooler_pin, GPIO.OUT, initial=GPIO.HIGH)  #Cooler
...
#WATER PERCENT
def moisture_percentage(adc_value, dry_value=1022, wet_value=965):
...
def ctrl_devices(temperature, humidity, soil_moisture, co2):
    ...
    # 0: led, 1: fan, 2: cooler, 3: water, 4: heater
    ...
    if led[1] == 1: #auto
        ...
        if led[0] == 0:
            ...
            print("human led on")
            GPIO.output(led_pin, GPIO.LOW)
        ctime = datetime.strptime(led[3],"%Y-%m-%d %H:%M:%S")
        if time.time() - ctime.timestamp() >= led[2]:
    ...
    if water[1] == 1: 
        ...
        ctime = datetime.strptime(water[3],"%Y-%m-%d %H:%M:%S")
        if time.time() - ctime.timestamp() >= water[2]:
    ...
    if fan[1] == 1:#auto
        ...
        ctime = datetime.strptime(fan[3],"%Y-%m-%d %H:%M:%S")
        if time.time() - ctime.timestamp() >= fan[2]:
    ...
    if heater[1] == 1: #auto
        ...
        ctime = datetime.strptime(heater[3],"%Y-%m-%d %H:%M:%S")
        if time.time() - ctime.timestamp() >= heater[2]:
    ...
    if cooler[1] == 1:
        ...
        ctime = datetime.strptime(cooler[3],"%Y-%m-%d %H:%M:%S")
        if time.time() - ctime.timestamp() >= cooler[2]:
