    ...
    async function loadFarmName() {
      try {
        ...
        if (storedFarmName) {
          ...
          console.log('[BottomTabBar] 내부저장소 SELECTED_FARM_NAME:', storedFarmName);
        } else {
          console.log('[BottomTabBar] farmName 없음');
        }
      } catch (err) {
        console.log('[BottomTabBar] farmName 로드 오류:', err);
      }
    }
    loadFarmName();
  }, []);
